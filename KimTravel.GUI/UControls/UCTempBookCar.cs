...
namespace KimTravel.GUI.UControls
{
    public partial class UCTempBookCar : XtraUserControl
    {
        ...
        private int _FromCarIndex = 9;
        private int _indexTourSelected = 0;
        public UCTempBookCar()
        {
            InitializeComponent();
        }
        private void loadDataGroup()
        {
            tempBCService = new TempBookCarService();
        ...
        private void UCGroupTour_Load(object sender, EventArgs e)
        {
            ...
            _tableTemp.Columns.Add("Added", typeof(bool));
            _tableTemp.Columns.Add("Car1", typeof(bool));
            _tableTemp.Columns.Add("Car2", typeof(bool));
            _tableTemp.Columns.Add("Car3", typeof(bool));
            _tableTemp.Columns.Add("Car4", typeof(bool));
            _tableTemp.Columns.Add("Car5", typeof(bool));
            _tableTemp.Columns.Add("Car6", typeof(bool));
            _tableTemp.Columns.Add("Car7", typeof(bool));
            _tableTemp.Columns.Add("Car8", typeof(bool));
            _tableTemp.Columns.Add("Car9", typeof(bool));
            _tableTemp.Columns.Add("Car10", typeof(bool));
        ...
        private void dataGridViewGroupTour_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (senderGrid.DataSource != null)
            {
                if (senderGrid.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString() == "True")
                {
                    ...
                    for (int i = _FromCarIndex; i < senderGrid.Columns.Count; i++)
                    {
                        if (i != e.ColumnIndex)
                ...
                int bookID = int.Parse(senderGrid.Rows[e.RowIndex].Cells["colID"].Value.ToString());
                string carName = senderGrid.Columns[e.ColumnIndex].Name;
                tempBCService.UpdateBookSelectedCar(bookID, carName);
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            gridControlData.DataSource = tempBCService.GetListBookedNotInCar(gID, dateStart);
            ...
            for (int i = 0; i < gridViewData.RowCount; i++)
            {
                ...
                if (i == 0)
                    _TourID = tID;
                var listData = tempBCService.GetListBooked(tID, dateStart, false);
                    ...
                    foreach (PropertyInfo prop in props)
                    {
                        ...
                        switch (key)
                        {
                            ...
                            case "Room":
                                ...
                                break;
                            case "ServiceName":
                                dr["ServiceName"] = propValue.ToString();
                                break;
                            case "PartnerPrice":
                            ...
                            case "Note":
                                ...
                                break;
                            case "Added":
                                dr["Added"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car1":
                                dr["Car1"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car2":
                                dr["Car2"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car3":
                                dr["Car3"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car4":
                                dr["Car4"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car5":
                                dr["Car5"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car6":
                                dr["Car6"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car7":
                                dr["Car7"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car8":
                                dr["Car8"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car9":
                                dr["Car9"] = bool.Parse(propValue.ToString());
                                break;
                            case "Car10":
                                dr["Car10"] = bool.Parse(propValue.ToString());
                                break;
                            default:
                                break;
                        }
        ...
        private void getDataRowChecked(string colName, int numCar)
        {
            ...
            data.Columns.Add("Added", typeof(bool));
            foreach (DataGridViewRow row in dataGridViewGroupTour.Rows)
            {
                if (row.Cells[colName].Value != null && (bool)row.Cells[colName].Value)
                {
                    ...
                    dr["Added"] = bool.Parse(row.Cells["colAdded"].Value.ToString());
        ...
        private void boSungDoiTac(DataTable data)
        {
            ...
            foreach (DataRow item in data.Rows)
            {
                ...
                int bookID = int.Parse(item["ID"].ToString());
                dr["ID"] = bookID;
                ...
                dr["Car10"] = false;
                dt.Rows.Add(dr);
                tempBCService.AddBookFromOtherTour(dateStart, bookID);
            }
        ...
        private void dataGridViewGroupTour_DataBindingComplete()
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridViewGroupTour.Rows)
                {
                    ...
                    bool check = false;
                    for (int i = _FromCarIndex; i < dataGridViewGroupTour.Columns.Count; i++)
                    {
        ...
        private void xoaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var colAdd = listTablePick[_indexTourSelected].Rows[index]["Added"].ToString();
            bool added = bool.Parse(colAdd);

            if (!added)
            {
                XtraMessageBox.Show("Chỉ được phép xóa những đối tác được bổ sung (ghép) vào Tour. Vui lòng kiểm tra lại.");
            }
            else
            {
                //Xóa dữ liệu trong bảng temp book car
                int bookID = int.Parse(listTablePick[_indexTourSelected].Rows[index]["ID"].ToString());
                tempBCService.RemoveBookCarAdded(bookID);
        ...
        private void lamMơiToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (DialogResult.OK == XtraMessageBox.Show("Bạn muốn xóa tất cả các đối tác đã chọn. Sắp xếp lại từ đầu ?", "Xác nhận", MessageBoxButtons.OKCancel))
            {
                ...
                {
                    DataRow dr = dt.NewRow();
                    int bookID = int.Parse(row.Cells["colID"].Value.ToString());
                    dr["ID"] = bookID;
                    dr["Pax"] = float.Parse(row.Cells["colPax"].Value.ToString());
                    dr["PaxChild"] = float.Parse(row.Cells["colPaxChild"].Value.ToString());
                    dr["PickUp"] = row.Cells["colPickUp"].Value.ToString();
                    ...
                    dr["PartnerPrice"] = int.Parse(row.Cells["colPartnerPrice"].Value.ToString());
                    dr["Note"] = row.Cells["colNote"].Value.ToString();
                    dr["Added"] = row.Cells["colAdded"].Value.ToString();
                    dr["Car1"] = false;
                    dr["Car2"] = false;
                    dr["Car3"] = false;
                    dr["Car4"] = false;
                    ...
                    tempBCService.UpdateBookSelectedCar(bookID, "");
                }
                listTablePick[_indexTourSelected] = dt;
                listTablePick[_indexTourSelected].AcceptChanges();
                dataGridViewGroupTour.DataSource = listTablePick[_indexTourSelected];
